var Whizzie = (function($) {
  ...
  function PluginManager() {
        ...
        if (typeof response.url != 'undefined') {
          ...
          if (response.hash == current_item_hash) {
            ...
            jQuery.post(response.url, response, function(response2) {
              process_current();
              $current_node.find('.wizard-plugin-status').text(response.message + mizan_importer_pro_whizzie_params.verify_text);
            }).fail(ajax_callback);
    ...
    function process_current() {
      if (current_item) {
        ...
        jQuery.post(mizan_importer_pro_whizzie_params.ajaxurl, {
  ...
  function WidgetManager(demo_type) {
    ...
    if (demo_type == 'builder') {
      ...
      function import_widgets() {
        jQuery.post(
          mizan_importer_pro_whizzie_params.ajaxurl, {
      ...
      function import_widgets() {
        jQuery.post(
          mizan_importer_pro_whizzie_params.ajaxurl, {
            action: 'mizan_importer_setup_elementor',
            wpnonce: mizan_importer_pro_whizzie_params.wpnonce
  ...
  function get_client_mizan_elementor_templates() {
    jQuery.ajax({
      method: "POST",
      url: mizan_importer_wizard_script_params.WP_MIZAN_IMPORTER_LICENCE_ENDPOINT + "get_client_mizan_elementor_templates",
      data: JSON.stringify({
        "limit": 9,
        "start": 1,
        "search": ""
      ...
      if (data.success === true) {
        ...
        for (var i = 0; i < elementor_templates.length; i++) {
          ...
          if (elementor_template.is_paid == "1") {
            pro_badge = mizan_importer_wizard_script_params.pro_badge;
          }
        ...
        $('#elementor_templates').removeClass('mizan-demo-importer-spinning');
      }
    });
  }
 get_client_mizan_elementor_templates();
  ...
  function ThemeManager() {
          ...
          if (response.hash == current_item_hash) {
            ...
            if (response.message == "Installing Plugin") {
              jQuery.get(response.url, function(response2) {
                process_current();
                $current_node.find('span').text(response.message + mizan_importer_pro_whizzie_params.verify_text);
              }).fail(ajax_callback);
            } else {
              jQuery.get(response.url, function(response2) {
                process_current();
    ...
    function process_current() {
      if (current_item) {
        ...
        // if we don't get a reply we can assume everything worked and continue onto the next one.
        jQuery.post(mizan_importer_pro_whizzie_params.ajaxurl, {
          action: 'mizan_importer_setup_themes',
          wpnonce: mizan_importer_pro_whizzie_params.wpnonce,
          slug: current_item
        }, ajax_callback).fail(ajax_callback);
    ...
    function ImportManager() {
      function import_elementor() {
        jQuery.post(
          mizan_importer_wizard_script_params.ajaxurl, {
            action: 'mizan_importer_setup_elementor',
            wpnonce: mizan_importer_wizard_script_params.wpnonce,
      ...
      return {
        init: function(btn) {
          ...
          ajax_callback = function(response) {
            ...
            $('.mizan-demo-importer-demo-main-btn').removeClass('merlin__button--loading');
    ...
    function find_next(slug) {
      ...
      var $li = $('.mizan-demo-importer-do-themes li[data-slug="' + slug + '"]');
...
jQuery(document).ready(function() {
  ...
  if (current_menu == 1) {
    ...
    jQuery('#toplevel_page_mizan-wizard').addClass('current');
  }
  ...
  jQuery('#mizan_importer_pro_license_form button[id="change--key"]').on('click', function() {
    var $mizan_importer_pro_license_form = jQuery('#mizan_importer_pro_license_form');
    $mizan_importer_pro_license_form.find('input[name="mizan_importer_pro_license_key"]').val('');
    $mizan_importer_pro_license_form.find('input[name="mizan_importer_pro_license_key"]').attr('disabled', false);
    $mizan_importer_pro_license_form.find('button[type="submit"]').val('');
    $mizan_importer_pro_license_form.find('button[type="submit"]').attr('disabled', false);
    $mizan_importer_pro_license_form.find('button[type="submit"]').text('Activate');
    jQuery('#start-now-next').hide();
    jQuery(this).remove();
  });
  jQuery('form#mizan_importer_pro_license_form').on('submit', function(e) {
    jQuery('.wee_theme_activation_spinner').show();
    e.preventDefault();
    var key_to_send = jQuery('form#mizan_importer_pro_license_form').serializeArray()[0].value;
    if (key_to_send == "") {
      ...
      jQuery.post(
        mizan_importer_pro_whizzie_params.ajaxurl, {
          action: 'wz_activate_mizan_importer_pro',
          wpnonce: mizan_importer_pro_whizzie_params.wpnonce,
          mizan_importer_pro_license_key: key_to_send
        },
        ...
        function(data, status) {
          if (status == 'success') {
            if (data.status) {
              ...
              jQuery('form#mizan_importer_pro_license_form button[type="submit"]').css("background-color:#0a9d2c");
              jQuery('form#mizan_importer_pro_license_form button[type="submit"]').text('Activated');
              jQuery('form#mizan_importer_pro_license_form button[type="submit"]').attr('disabled', 'disabled');
...
jQuery("#start-now-next").click(function(){
