...
class Mizan_Importer_ThemeWhizzie
{
  ...
  public function enqueue_scripts($hook)
  {
    ...
    $is_mizan_author = (strtolower($theme_author) === 'mizanthemes');
    $free_theme_constant = strtoupper(str_replace('-', '_', $theme_text_domain)) . '_DOCUMENTATION';
    $has_free_theme_constant = defined($free_theme_constant);
    $is_free_theme = $is_mizan_author && $has_free_theme_constant;
    ...
    wp_localize_script('theme-wizard-script', 'mizan_importer_pro_whizzie_params', array(
      ...
      'verify_text' => esc_html('verifying', 'mizan-demo-importer'),
      ...
      <div class="row my-5 align-items-center">
        <div class="col-md-10">
          ...
          <p class="main-grid-banner-para">
            <?php echo esc_html('Explore The Ultimate Collection of Free Elementor WordPress Themes'); ?>
          </p>
        </div>
        <div class="col-md-2">
    ...
    <div class="wp-setup-finish">
      ...
      <div class="finish-buttons">
        <a href="<?php echo esc_url(admin_url('/customize.php')); ?>" class="wz-btn-customizer"
          target="_blank"><?php esc_html_e('Customize Your Demo', 'mizan-demo-importer') ?></a>
        <a href="" class="wz-btn-builder"
          target="_blank"><?php esc_html_e('Customize Your Demo', 'mizan-demo-importer'); ?></a>
        <a href="<?php echo esc_url(site_url()); ?>" class="wz-btn-visit-site"
          target="_blank"><?php esc_html_e('Visit Your Site', 'mizan-demo-importer'); ?></a>
      </div>
      <div class="wp-finish-btn">
        <a href="<?php echo esc_url(admin_url()); ?>" class="button button-primary" onclick="openCity(event, 'theme_info')"
    ...
    foreach ($plugins['activate'] as $slug => $plugin) {
      if ($_POST['slug'] == $slug) {
        $json = array(
          ...
          'action2' => -1,
          'message' => esc_html__('Activating Plugin', 'mizan-demo-importer')
        );
        break;
      }
    }
    foreach ($plugins['update'] as $slug => $plugin) {
      if ($_POST['slug'] == $slug) {
        $json = array(
          ...
          'plugin_status' => 'all',
          '_wpnonce' => wp_create_nonce('bulk-plugins'),
          'action' => 'mizan-demo-importer-tgmpa-bulk-update',
          'action2' => -1,
          'message' => esc_html__('Updating Plugin', 'mizan-demo-importer')
        );
        break;
      }
    }
    foreach ($plugins['install'] as $slug => $plugin) {
      if ($_POST['slug'] == $slug) {
        $json = array(
          ...
          'plugin_status' => 'all',
          '_wpnonce' => wp_create_nonce('bulk-plugins'),
          'action' => 'mizan-demo-importer-tgmpa-bulk-install',
          'action2' => -1,
          'message' => esc_html__('Installing Plugin', 'mizan-demo-importer')
        );
        break;
      }
    }
    ...
    <div class="main-grid-banner-parent">
      <div class="row my-5 align-items-center">
        <div class="col-md-10">
          <h2 class="main-grid-banner-head"><?php echo esc_html('Mizan Themes,'); ?></h2>
          <p class="main-grid-banner-para">
            <?php echo esc_html('Explore The Ultimate Collection of Premium Elementor WordPress Themes'); ?>
          </p>
        </div>
        <div class="col-md-2">
          <img class="main-grid-banner-logo img-fluid"
    ...
    <div class="main-grid-card-parent">
          ...
          <div class="dropdown">
            ...
            <ul class="dropdown-menu">
              ...
              <?php foreach ($product_cat_arr as $key => $single_cat) {
                ...
                ?>
                <li class="d-flex justify-content-space-between align-items-center">
                  <a class="dropdown-item templates p-0" href="#"
                    data-category="<?php echo esc_attr($single_cat->handle); ?>"><?php echo esc_html($single_cat->title); ?></a>
        ...
        if (isset($themes_arr['themes'])) {
          foreach ($themes_arr['themes'] as $key => $theme) {
            ...
            <div class="main-grid-card-parent col-lg-4 col-md-6 col-12">
              <div class="main-grid-card-parent-inner">
                <div class="main-grid-card-parent-inner-image-head">
                  <img class="main-grid-card-parent-inner-image" src="<?php echo esc_url($thumbnail_url); ?>" width="100"
                    height="100" alt="<?php echo esc_url($get_the_title); ?>">
                </div>
                <div class="main-grid-card-parent-inner-description">
                  <h3><?php echo esc_html($get_the_title); ?></h3>
                  <div class="main-grid-card-parent-inner-button">
                    <a target="_blank" href="<?php echo esc_url($product_permalink); ?>"
                      class="main-grid-card-parent-inner-button-buy"><?php echo esc_html('Buy Now'); ?></a>
                    <a target="_blank" href="<?php echo esc_url($live_demo); ?>"
                      class="main-grid-card-parent-inner-button-preview"><?php echo esc_html('Demo'); ?></a>
                  </div>
                </div>
              </div>
            </div>
      ...
      <?php if ($themes_arr['total_pages']->hasNextPage) { ?>
        <div class="main-grid-card-load-more-parent text-center my-2">
          <input type="hidden" name="load_more"
            value="<?php echo esc_attr(isset($themes_arr['total_pages']->endCursor) ? $themes_arr['total_pages']->endCursor : ''); ?>">
  ...
  public function mizan_importer_pro_mostrar_guide()
  {
    ...
    if (
      ...
      $endpoint = MDI_WPEI_SHOPIFY_LICENCE_ENDPOINT . 'status';
    } else {
      ...
      <div class="wee-tab-sec wee-theme-option-tab">
        <div class="wee-tab">
          ...
          if (defined('GET_PREMIUM_THEME')) {
            ?>
            <div class="tab">
              <button class="tablinks active" onclick="openCity(event, 'wee_theme_activation')"
                data-tab="wee_theme_activation"><?php _e('Key Activation', 'mizan-demo-importer'); ?></button>
            </div>
          <?php } ?>
        </div>
        <!-- Tab content -->
